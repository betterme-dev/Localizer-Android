...
internal class TranslationsRestStoreImpl(
    ...
    override fun loadTranslationsUrls(
        locales: List<String>,
        filters: List<String>,
        tags: List<String>,
        apiParams: ApiParams
    ...
    override fun loadTranslationsUrl(
        locale: String,
        filters: List<String>,
        tags: List<String>,
        apiParams: ApiParams
    ): String {
        val postRequest = createTranslationsExportRequest(
            locale,
            apiParams.apiToken,
            apiParams.projectId,
            filters,
    ...
    override fun getAvailableLanguages(apiParams: ApiParams): List<String> {
        ...
        val languageResponse =
            gson.fromJson(languageResponseString, AvailableLanguagesResponse::class.java)
        return languageResponse.result.languages.map { it.code }
    }

    override fun uploadStringsFile(
        apiParams: ApiParams,
        filePath: String,
        locale: String,
        overWrite: Boolean,
        ...
        val fileUploadingRequest = createTranslationsUploadRequest(
            apiParams.apiToken,
        ...
        if (!fileResponse.isSuccessful) throw IOException()
        val fileResponseString = fileResponse.body?.string() ?: throw IllegalStateException(
            "Could " +
                    "not upload translations file"
        )
        val uploadResponse =
            gson.fromJson(fileResponseString, TranslationsUploadResponse::class.java)
        println("Translations file $filePath upload response: $uploadResponse")
        return
    }

    private fun createTranslationsExportRequest(
        locale: String,
        apiToken: String,
        projectId: String,
        filters: List<String>,
        ...
        val requestBodyBuilder = FormBody.Builder()
            ...
            .add(
                MetaDataContants.Params.PARAM_TYPE,
                MetaDataContants.Values.VALUE_TYPE_ANDROID_STRINGS
            )
    ...
    private fun createTranslationsUploadRequest(
        ...
        val fileRequestBody = MultipartBody.Builder()
            ...
            .addFormDataPart(
                "file", filePath, File(filePath).asRequestBody("xml".toMediaTypeOrNull())
            )
            ...
            .addFormDataPart(
                MetaDataContants.Params.PARAM_UPDATING,
